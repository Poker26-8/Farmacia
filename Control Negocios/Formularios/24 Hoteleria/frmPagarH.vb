...
Public Class frmPagarH
    ...
    Private Sub frmPagarH_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            cmd2 = cnn2.CreateCommand
            cmd2.CommandText = "SELECT id,Codigo,Nombre,Cantidad,Precio,Total,IDC FROM comandas WHERE NMESA='" & lblHabitacion.Text & "'"
            rd2 = cmd2.ExecuteReader
            Do While rd2.Read
                If rd2.HasRows Then
            ...
            If IsNumeric(txtnumref.Text) Then
                ...
                If cbotpago.Text = "MONEDERO" Then
                    Try
                        cnn1.Close() : cnn1.Open()
                        cmd1 = cnn1.CreateCommand
                        cmd1.CommandText = "SELECT Saldo FROM Monedero WHERE Barras='" & txtnumref.Text & "'"
                        rd1 = cmd1.ExecuteReader
                        If rd1.HasRows Then
                            If rd1.Read Then
            ...
            For dekua As Integer = 0 To grdpago.Rows.Count - 1
                ...
                If tipopago = "MONEDERO" Then
                    ...
                    If foliomonedro <> "" Then
                        cnn2.Close() : cnn2.Open()
                        cmd2 = cnn2.CreateCommand
                        cmd2.CommandText = "SELECT Id,Saldo,Porcentaje FROM Monedero WHERE Barras='" & foliomonedro & "'"
                        rd2 = cmd2.ExecuteReader
                        If rd2.HasRows Then
                            If rd2.Read Then
...
#End Region
        ...
        Select Case lblTipoVenta.Text
            ...
            Case Is <> "MOSTRADOR"
                ...
                cmd3 = cnn3.CreateCommand
                cmd3.CommandText =
                    "insert into Ventas(IdCliente,Cliente,Direccion,Subtotal,IVA,Totales,Descuento,Devolucion,ACuenta,Resta,Usuario,FVenta,HVenta,FPago,FCancelado,Status,Comisionista,Concepto,MontoSinDesc,FEntrega,Comentario,StatusE,IP,Propina,CodFactura,Formato,Fecha) values(" & lblNumCliente.Text & ",'" & lblCliente.Text & "',''," & MYSUBTOTALVE & "," & tIVA & "," & TOTALVENTA & "," & DESCUENTOVENTA & ",0," & ACUENTA2 & "," & RESTAVENTA & ",'" & lblAtendio.Text & "','" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "HH:mm:ss") & "','','','" & MyStatus & "','',''," & DESCUENTOVENTA & ",'','',0,'" & dameIP2() & "',0,'" & lic & "','TICKET','" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "')"
                cmd3.ExecuteNonQuery()
                cnn3.Close()
        End Select
        ...
        Try
            ...
            If CantidadP <> "" Then
                If cantidadeliminar >= CantidadP Then
                    ...
                    cnn3.Close() : cnn3.Open()
                    cmd3 = cnn3.CreateCommand
                    cmd3.CommandText = "SELECT UVenta,PrecioCompra,PrecioVentaIVA,IVA,Departamento,Grupo FROM Productos WHERE Codigo='" & codigoeliminar & "'"
                    rd3 = cmd3.ExecuteReader
                    If rd3.Read Then
                        If rd3.HasRows Then

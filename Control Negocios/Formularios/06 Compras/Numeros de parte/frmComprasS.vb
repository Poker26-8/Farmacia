...
Public Class frmComprasS
    ...
    Private Sub frmComprasS_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cmd1 = cnn1.CreateCommand
        cmd1.CommandText =
            "select Rem,Fac,Ped,Proveedor,Codigo,Nombre,Unidad,Cantidad,Precio,CP from AuxComprasSeries order by Id"
        rd1 = cmd1.ExecuteReader
        Do While rd1.Read
            If rd1.HasRows Then
                ...
                grdcaptura.Rows.Add(codigo, nombre, unidad, cantidad, FormatNumber(precio, 4), FormatNumber(total, 2), existencia, cp)
    ...
    Private Sub cboproveedor_DropDown(sender As System.Object, e As System.EventArgs) Handles cboproveedor.DropDown
        ...
        Try
            ...
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
                If rd1.HasRows Then cboproveedor.Items.Add(rd1(0).ToString())
            Loop
            rd1.Close()
            cnn1.Close()
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            If txtcodigo.Text <> "" Then
                Try
                    ...
                    cmd1 = cnn1.CreateCommand
                    cmd1.CommandText =
                        "select Codigo,UCompra,Nombre,PrecioCompra,Multiplo from Productos where Codigo='" & txtcodigo.Text & "' and Departamento<>'SERVICIOS'"
                    rd1 = cmd1.ExecuteReader
                    If rd1.HasRows Then
                        If rd1.Read Then
    ...
    Private Sub cboremision_DropDown(sender As Object, e As System.EventArgs) Handles cboremision.DropDown
        ...
        Try
            ...
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
                If rd1.HasRows Then cboremision.Items.Add(rd1(0).ToString())
            Loop
            rd1.Close()
            cnn1.Close()
        Catch ex As Exception
                ...
                Do While rd1.Read
                    If rd1.HasRows Then
                        ...
                        cmd2 = cnn2.CreateCommand : cmd2.CommandText =
                            "select Nombre,UCompra,Multiplo,Existencia,Precio,Cantidad from Productos where Codigo='" & codigo & "'"
                        rd2 = cmd2.ExecuteReader
                        If rd2.HasRows Then
                            If rd2.Read Then
                        ...
                        grdcaptura.Rows.Add(codigo, nombre, unidad, cantidad, FormatNumber(precio, 4), FormatNumber(total, 2), existencia, "")
    ...
    Private Sub cbofactura_DropDown(sender As System.Object, e As System.EventArgs) Handles cbofactura.DropDown
        ...
        Try
            ...
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
                If rd1.HasRows Then cbofactura.Items.Add(rd1(0).ToString())
            Loop
            rd1.Close()
            cnn1.Close()
        Catch ex As Exception
    ...
    Private Sub cbofactura_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles cbofactura.KeyPress
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            Try
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText =
                    "select Codigo,Precio,Cantidad from ComprasDet where NumFactura='" & cbofactura.Text & "' and Proveedor='" & cboproveedor.Text & "'"
                rd1 = cmd1.ExecuteReader
                Do While rd1.Read
                    If rd1.HasRows Then
                        Dim codigo As String = rd1("Codigo").ToString()
    ...
    Private Sub cbopedido_DropDown(sender As Object, e As System.EventArgs) Handles cbopedido.DropDown
        ...
        Try
            ...
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
                If rd1.HasRows Then cbopedido.Items.Add(rd1(0).ToString())
            Loop
            rd1.Close()
            cnn1.Close()
        Catch ex As Exception
            ...
            Try
                ...
                Do While rd1.Read
                    If rd1.HasRows Then
                        ...
                        grdcaptura.Rows.Add(codigo, nombre, unidad, cantidad, FormatNumber(precio, 4), FormatNumber(total, 2), existencia, "0")
    ...
    Public Sub UpGrid(ByVal CP As Integer)
        ...
        'Grid
        grdcaptura.Rows.Add(txtcodigo.Text, cbonombre.Text, txtunidad.Text, txtcantidad.Text, FormatNumber(txtprecio.Text, 4), FormatNumber(txttotal.Text, 2), txtexiste.Text, CP)
        ...
        cmd1.CommandText =
            "insert into AuxComprasSeries(Rem,Fac,Ped,Proveedor,Codigo,Nombre,Unidad,Cantidad,Precio,Total,CP) values('" & cboremision.Text & "','" & cbofactura.Text & "','" & cbopedido.Text & "','" & cboproveedor.Text & "','" & txtcodigo.Text & "','" & cbonombre.Text & "','" & txtunidad.Text & "'," & CantidadP & "," & PrecioU & "," & TotalP & "," & CP & ")"
        cmd1.ExecuteNonQuery() : cnn1.Close()
    ...
    Private Sub txtmonto_KeyPress(sender As System.Object, e As System.Windows.Forms.KeyPressEventArgs) Handles txtmonto.KeyPress
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            grdPagos.Rows.Add(cbotpago.Text, cbobanco.Text, txtrefe.Text, FormatDateTime(dtppago.Value, DateFormat.ShortDate), txtmonto.Text)
    ...
    Private Sub grdcaptura_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles grdcaptura.KeyDown
        ...
        If e.KeyCode = Keys.Delete Then
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "delete from AuxComprasSeries where Codigo='" & codigo & "' and Cantidad=" & cantidad
    ...
    Private Sub grdcaptura_CellDoubleClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdcaptura.CellDoubleClick
        ...
        If grdcaptura.Rows.Count > 0 Then
            ...
            Else
                cnn1.Close() : cnn1.Open() : cmd1 = cnn1.CreateCommand
                cmd1.CommandText =
                    "delete from AuxComprasSeries where Codigo='" & txtcodigo.Text & "' and Cantidad=" & txtcantidad.Text
                cmd1.ExecuteNonQuery() : cnn1.Close()
            End If
        ...
        Try
            ...
            Else
                If CDbl(txtresta.Text) = 0 Then
                    ...
                    cmd1 = cnn1.CreateCommand
                    cmd1.CommandText =
                        "insert into AbonoE(NumRemision,NumFactura,NumPedido,IdProv,Proveedor,Concepto,Fecha,FechaCompleta,Hora,Cargo,Abono,Saldo,Banco,Referencia,Usuario) values('" & cboremision.Text & "','" & cbofactura.Text & "','" & cbopedido.Text & "'," & MyID & ",'" & cboproveedor.Text & "','COMPRA','" & Format(dtpfecha.Value, "yyyy-MM-dd") & "','" & Format(Date.Now, "HH:mm:ss") & "','" & Format(dtpfecha.Value, "yyyy-MM-dd HH:mm:ss") & "'," & CDbl(txtapagar.Text) & ",0," & MySaldo & ",'','','" & alias_compras & "')"
                    cmd1.ExecuteNonQuery()
                ...
                Else
                    ...
                    If MySaldoF > CDbl(txtresta.Text) Then
                        ...
                        cmd1 = cnn1.CreateCommand
                        cmd1.CommandText =
                            "insert into AbonoE(NumRemision,NumFactura,NumPedido,IdProv,Proveedor,Concepto,Fecha,Hora,FechaCompleta,Cargo,Abono,Saldo,Efectivo,Tarjeta,Transfe,Otro,Banco,Referencia,Usuario,Corte,CorteU,Cargado) values('" & cboremision.Text & "','" & cbofactura.Text & "','" & cbopedido.Text & "'," & MyID & ",'" & cboproveedor.Text & "','ABONO','" & Format(dtpfecha.Value, "yyyy-MM-dd") & "','" & Format(Date.Now, "HH:mm:ss") & "','" & Format(dtpfecha.Value, "yyyy-MM-dd HH:mm:ss
                            .") & "',0," & MyACuenta & "," & MySaldo & "," & efectivo & "," & tarjeta & "," & transfe & "," & MyACuenta & ",'" & banco & "','" & refer & "','" & alias_compras & "',0,0,0)"
                        cmd1.ExecuteNonQuery()
            ...
            Do While grdcaptura.Rows.Count <> Zi
                ...
                Dim cp As Boolean = IIf(grdcaptura.Rows(Zi).Cells(7).Value.ToString() = "", 0, grdcaptura.Rows(Zi).Cells(7).Value.ToString())
    ...
    Private Sub pTicket80_PrintPage(sender As System.Object, e As System.Drawing.Printing.PrintPageEventArgs) Handles pTicket80.PrintPage
        ...
        For miku As Integer = 0 To grdcaptura.Rows.Count - 1
            ...
            'Dim lote As String = grdcaptura.Rows(miku).Cells(8).Value
    ...
    Private Sub pTicket58_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles pTicket58.PrintPage
        ...
        If tLogo <> "SIN" Then
            If File.Exists(My.Application.Info.DirectoryPath & "\" & nLogo) Then
                ...
                End If
                If tLogo = "RECT" Then
                    e.Graphics.DrawImage(Logotipo, 5, 0, 110, 110)
                    Y += 120
                End If
            End If
        Else
        ...
        If rd1.HasRows Then
            If rd1.Read Then
                ...
                'Razón social
                If rd1("Cab0").ToString() <> "" Then
                    e.Graphics.DrawString(rd1("Cab0").ToString, New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 90, Y, sc)
                    Y += 12.5
                End If
                'RFC
                If rd1("Cab1").ToString() <> "" Then
                    e.Graphics.DrawString(rd1("Cab1").ToString, New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 90, Y, sc)
                    Y += 12.5
                End If
                'Calle  N°.
                If rd1("Cab2").ToString() <> "" Then
        ...
        Y += 15
        If tipo_impre = "NORMAL" Then
            e.Graphics.DrawString("C O M P R A", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 90, Y, sc)
            Y += 12
        End If
        If tipo_impre = "COPIA" Then
            e.Graphics.DrawString("C O P I A - C  O M P R A", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 140, Y, sc)
            Y += 12
        End If
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        Y += 18
        ...
        e.Graphics.DrawString("PRODUCTO", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 130, Y, sc)
        Y += 11
        e.Graphics.DrawString("CANTIDAD", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 1, Y)
        e.Graphics.DrawString("PRECIO U.", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 130, Y, sf)
        e.Graphics.DrawString("TOTAL", New Drawing.Font(tipografia, 9, FontStyle.Bold), Brushes.Black, 180, Y, sf)
        Y += 6
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        Y += 18
        ...
        For miku As Integer = 0 To grdcaptura.Rows.Count - 1
            ...
            e.Graphics.DrawString("x", fuente_prods, Brushes.Black, 32, Y)
        ...
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        Y += 15
        e.Graphics.DrawString("TOTAL DE PRODUCTOS " & txtprods.Text, New Drawing.Font(tipografia, 7, FontStyle.Bold), Brushes.Black, 140, Y, sc)
        Y += 7
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        Y += 18

        e.Graphics.DrawString("Subtotal:", fuente_prods, Brushes.Black, 1, Y)
        e.Graphics.DrawString(simbolo & FormatNumber(txtsub1.Text, 2), fuente_prods, Brushes.Black, 180, Y, sf)
        Y += 13.5

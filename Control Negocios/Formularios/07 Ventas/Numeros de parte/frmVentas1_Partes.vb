    ...
    Private Function ReviewLote() As Boolean
        ...
        If cboLote.Text <> "" Then
            ...
            Try
                ...
                cmd5 = cnn5.CreateCommand
                cmd5.CommandText =
                    "select Id,Caducidad from LoteCaducidad where Codigo='" & cbocodigo.Text & "' and Lote='" & cboLote.Text & "' and Cantidad>0"
                rd5 = cmd5.ExecuteReader
                If rd5.HasRows Then
                    If rd5.Read Then
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            Try
                ...
                If basculaxd = "Etiquetas" Then
                    ...
                    cmd1 = cnn1.CreateCommand
                    cmd1.CommandText =
                            "select Status_Promocion,Grupo,Departamento,Codigo,Nombre,UVenta,MCD,Min,Ubicacion from Productos where Codigo='" & codrecortado & "'"
                    rd1 = cmd1.ExecuteReader
                    If rd1.HasRows Then
                        If rd1.Read Then
...
kaka:
                ...
                Else
                    ...
                    If cbocodigo.Text <> "" Then
                        ...
                        cmd2 = cnn2.CreateCommand
                        cmd2.CommandText =
                            "select Status_Promocion,Grupo,Departamento,Codigo,Nombre,UVenta,Multiplo,Min,Ubicacion from Productos where Codigo='" & cbocodigo.Text & "'"
                        rd2 = cmd2.ExecuteReader
                        If rd2.HasRows Then
                            If rd2.Read Then
    ...
    Private Sub cboLote_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cboLote.KeyPress
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "select Departamento,Grupo from Productos where Codigo='" & cbocodigo.Text & "'"
            rd1 = cmd1.ExecuteReader
            If rd1.HasRows Then
                If rd1.Read Then
                    If CStr(rd1("Departamento").ToString()) = "SERVICIOS" Then
        ...
        Try
            Select Case lblNumCliente.Text
                ...
                Case Is <> "MOSTRADOR"
                    ...
                    cmd1 = cnn1.CreateCommand
                    cmd1.CommandText =
                        "insert into Ventas(IdCliente,Cliente,Direccion,Subtotal,IVA,Totales,Descuento,Devolucion,ACuenta,Resta,Usuario,FVenta,HVenta,FPago,FCancelado,Status,Comisionista,Concepto,MontoSinDesc,FEntrega,Entrega,Comentario,StatusE,FolMonedero,CodFactura,IP,Formato,Franquicia,Fecha) values(" & IdCliente & ",'" & cboNombre.Text & "','" & txtdireccion.Text & "'," & SubTotal & "," & IVA_Vent & "," & Total_Ve & "," & Descuento & ",0," & ACUenta2 & "," & Resta & ",'" & lblusuario.Text & "','" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "HH:mm:ss") & "','" & fecha_pago & "','','" & MyStatus & "','" & cbocomisionista.Text & "',''," & MontoSDesc & ",'" & Format(dtpFecha_E.Value, "dd/MM/yyyy") & "'," & IIf(Entrega = True, 1, 0) & ",'',0,'" & txtMonedero.Text & "','" & CodCadena & "','" & dameIP2() & "','" & cboimpresion.Text & "'," & validafranquicia & ",'" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "')"
                    cmd1.ExecuteNonQuery()
                    cnn1.Close()
            End Select
        Catch ex As Exception
            ...
            If ACuenta > 0 Then
                ...
                If grdpago.Rows.Count > 0 Then
                    For t As Integer = 0 To grdpago.Rows.Count - 1
                        ...
                        FormaPago = grdpago.Rows(t).Cells(0).Value.ToString()
                        If CStr(grdpago.Rows(t).Cells(0).Value.ToString()) = FormaPago Then
                            TotFormaPago = CDbl(grdpago.Rows(t).Cells(3).Value.ToString())
...
ecomoda:
    ...
    Private Sub cbocedula_SelectedValueChanged(sender As Object, e As EventArgs) Handles cbocedula.SelectedValueChanged
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "select Id,Nombre,Direccion from CtMedicos where Cedula='" & cbocedula.Text & "'"
            rd1 = cmd1.ExecuteReader
            If rd1.HasRows Then
                If rd1.Read Then
    ...
    Private Sub pVenta58_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles pVenta58.PrintPage
        ...
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "select Pie3,Cab0,Cab1,Cab2,Cab4,Cab5,Cab6 from Ticket"
            rd1 = cmd1.ExecuteReader
            If rd1.HasRows Then
                If rd1.Read Then
                    Pie = rd1("Pie3").ToString

Public Class frmCorte
    ...
    Private Sub btnCalcularGlobal_Click(sender As Object, e As EventArgs) Handles btnCalcularGlobal.Click
        ...
        Try
            ...
            cmd2 = cnn2.CreateCommand
            cmd2.CommandText =
                "select sum(Monedero) from AbonoI where Concepto<>'ABONO' and Concepto<>'CARGO' and Fecha='" & Format(dtpFecha.Value, "yyyy-MM-dd") & "'"
            rd2 = cmd2.ExecuteReader
            If rd2.HasRows Then
                If rd2.Read Then
                    txtEgrMonederoG.Text = IIf(rd2(0).ToString = "", "0.00", rd2(0).ToString)
                    CanceDevo = CDec(CanceDevo) + CDec(IIf(rd2(0).ToString = "", "0.00", rd2(0).ToString))
                End If
            End If
            rd2.Close()

            txtEgrMonederoG.Text = FormatNumber(txtEgrMonederoG.Text, 2)

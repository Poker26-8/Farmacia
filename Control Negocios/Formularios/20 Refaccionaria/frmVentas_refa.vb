...
kak:
    ...
    Private Sub ActualizaPEPS(ByVal folio As String, ByVal codigo As String, ByVal cantidad As Double)
        ...
        Try
            ...
            cmd4 = cnn4.CreateCommand
            cmd4.CommandText = "SELECT Nombre,UVenta FROM Productos WHERE Codigo='" & codigo & "'"
            rd4 = cmd4.ExecuteReader
            If rd4.HasRows Then
                If rd4.Read Then
...
kaka:
                ...
                '---------------------------------------NUMPARTE
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText = "SELECT Status_Promocion,Grupo,Departamento,Codigo,Nombre,UVenta,Multiplo,Min,Ubicacion FROM Productos WHERE N_Serie='" & cbodesc.Text & "'"
                rd1 = cmd1.ExecuteReader
                If rd1.HasRows Then
                    If rd1.Read Then
                        Promo = IIf(rd1("Status_Promocion").ToString = False, False, True)
    ...
    Private Sub txtcantidad_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtcantidad.KeyPress
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            Try
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText = "SELECT Vent_EPrec FROM Permisos WHERE IdEmpleado=" & id_usu_log
                rd1 = cmd1.ExecuteReader
                If rd1.HasRows Then
                    If rd1.Read Then
    ...
    Private Sub cbobanco_DropDown(sender As Object, e As EventArgs) Handles cbobanco.DropDown
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText = "select Banco from Bancos"
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            If cbotpago.Text = "OTRO" Then
                Try
                    ...
                    cmd1 = cnn1.CreateCommand
                    cmd1.CommandText =
                        "select Id from MovCuenta where Tipo='OTRO' and Referencia='" & txtnumref.Text & "'"
                    rd1 = cmd1.ExecuteReader
                    If rd1.HasRows Then
                        If rd1.Read Then
                            MsgBox("Ya fue registrado este pago en una venta diferente.", vbInformation + vbOKOnly, "Delsscom Control Negocios Pro")
...
ecomoda:
    ...
    Private Sub pVentaCarta_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles pVentaCarta.PrintPage
        ...
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "select Pie2,Cab0,Cab1,Cab2,Cab3,Cab4,Cab5,Cab6,Pagare from Ticket"
            rd1 = cmd1.ExecuteReader
            If rd1.HasRows Then
                If rd1.Read Then
                    Pie = rd1("Pie2").ToString
    ...
    Private Sub txtparte_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtparte.KeyPress
        ...
        If AscW(e.KeyChar) = Keys.Enter Then
            ...
            Try
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText = "SELECT Servicio,Nombre FROM refaccionaria WHERE NumParte='" & txtparte.Text & "'"
                rd1 = cmd1.ExecuteReader
                If rd1.HasRows Then
                    If rd1.Read Then
    ...
    Private Sub cbonota_LostFocus(sender As Object, e As EventArgs) Handles cbonota.LostFocus
        If cbonota.Text <> "" Then
            ...
            Try
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText = "SELECT IdCliente,Cliente FROM Ventas WHERE Folio=" & cbonota.Text
                rd1 = cmd1.ExecuteReader
                If rd1.HasRows Then
                    If rd1.Read Then

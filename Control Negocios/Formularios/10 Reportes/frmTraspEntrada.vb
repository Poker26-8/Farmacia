...
Public Class frmTraspEntrada
    Dim renglon As Integer = 0
    Dim usu_copia As String = ""

    Private Sub frmTraspEntrada_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Folio()
    ...
    Public Sub Folio()
        Try
            ...
            cmd3.CommandText =
                "select max(Folio) from Traslados where Concepto='ENTRADA'"
    ...
    Private Sub cbo_DropDown(sender As System.Object, e As System.EventArgs) Handles cbo.DropDown
        ...
        Try
            ...
            cmd1.CommandText =
                "select distinct Nombre from Traslados where Concepto='ENTRADA'"
    ...
    Private Sub btnguardar_Click(sender As System.Object, e As System.EventArgs) Handles btnguardar.Click
        ...
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "insert into Traslados(Nombre,Totales,FVenta,HVenta,Usuario,Folio,Concepto,Cargado,FPago,FCancelado,Comisionista) values('" & cbo.Text & "'," & MyTotal & ",'" & Format(dtpfecha.Value, "yyyy-MM-dd") & "','" & Format(Date.Now, "yyyy-MM-dd HH:mm:ss") & "','" & lblusuario.Text & "'," & lblfolio.Text & ",'ENTRADA',0,'" & Format(Date.Now, "yyyy-MM-dd") & "','" & Format(Date.Now, "yyyy-MM-dd") & "','" & cbo.Text & "')"
            ...
            For bombita As Integer = 0 To grdcaptura.Rows.Count - 1
                ...
                cmd1 = cnn1.CreateCommand
                cmd1.CommandText =
                    "insert into TrasladosDet(Id,Codigo,Nombre,Unidad,Cantidad,Precio,Total,Existe,Fecha,Concepto,Depto,Grupo,Folio,CostVR) values(" & MYFOLIO & ",'" & codigo & "','" & nombre & "','" & unidad & "'," & cantidad & "," & precio & "," & total & "," & existe & ",'" & Format(dtpfecha.Value, "yyyy-MM-dd HH:mm:ss") & "','ENTRADA','" & depto & "','" & grupo & "'," & lblfolio.Text & ",'')"
...
Nota:
    ...
    Private Sub pEntrada80_PrintPage(sender As System.Object, e As System.Drawing.Printing.PrintPageEventArgs) Handles pEntrada80.PrintPage
        ...
        Y += 17
        e.Graphics.DrawString("Procedencia: " & cbo.Text, fuente_datos, Brushes.Black, 1, Y)
        Y += 12
...
milky:
    ...
    Private Sub cbodocumento_DropDown(sender As System.Object, e As System.EventArgs) Handles cbodocumento.DropDown
        ...
        Try
            ...
            cmd1 = cnn1.CreateCommand
            cmd1.CommandText =
                "select distinct Folio from Traslados where Concepto='ENTRADA'"
            rd1 = cmd1.ExecuteReader
            Do While rd1.Read
    ...
    Private Sub btncopia_Click(sender As System.Object, e As System.EventArgs) Handles btncopia.Click
        If cbodocumento.Text = "" Then MsgBox("Selecciona un documento para imprimir la copia.", vbInformation + vbOKOnly, "Delsscom Control Negocios Pro") : cbodocumento.Focus().Equals(True) : Exit Sub
        If MsgBox("¿Deseas imprimir una copia del traslado?", vbInformation + vbOKCancel, "Delsscom Control Negocios Pro") = vbOK Then
            Dim TPrint As String = ""
            Dim Imprime_En As String = ""
            Dim Impresora As String = ""
            Dim Tamaño As String = ""
            ...
            If TPrint = "TICKET" Then
                ...
                End If
                If Tamaño = "58" Then
                    pCopia58.DefaultPageSettings.PrinterSettings.PrinterName = Impresora
                    pCopia58.Print()
                End If
            End If
            If TPrint = "MEDIA CARTA" Then
                pCopiaMc.DefaultPageSettings.PrinterSettings.PrinterName = Impresora
                pCopiaMc.Print()
            End If
            If TPrint = "CARTA" Then
                pCopiaCarta.DefaultPageSettings.PrinterSettings.PrinterName = Impresora
                pCopiaCarta.Print()
            End If
            If TPrint = "PDF - CARTA" Then
    ...
    Private Sub pCopia80_PrintPage(sender As System.Object, e As System.Drawing.Printing.PrintPageEventArgs) Handles pCopia80.PrintPage
        'Fuentes prederminadas
        Dim tipografia As String = "Lucida Sans Typewriter"
        Dim fuente_datos As New Drawing.Font(tipografia, 10, FontStyle.Regular)
        Dim fuente_prods As New Drawing.Font(tipografia, 9, FontStyle.Regular)
        ...
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        Y += 15
        e.Graphics.DrawString("C O P I A  T R A S P A S O", New Drawing.Font(tipografia, 11, FontStyle.Bold), Brushes.Black, 140, Y, sc)
        Y += 14
        e.Graphics.DrawString("E N T R A D A", New Drawing.Font(tipografia, 12, FontStyle.Bold), Brushes.Black, 140, Y, sc)
        Y += 12
        e.Graphics.DrawString("--------------------------------------------------------", New Drawing.Font(tipografia, 12, FontStyle.Regular), Brushes.Black, 1, Y)
        ...
        e.Graphics.DrawString("Folio: " & cbodocumento.Text, fuente_datos, Brushes.Black, 285, Y, sf)
        e.Graphics.DrawString("Fecha: " & FormatDateTime(dtpfecha.Value, DateFormat.ShortDate), fuente_prods, Brushes.Black, 1, Y)
        Y += 17
        e.Graphics.DrawString("Procedencia: " & cbo.Text, fuente_datos, Brushes.Black, 1, Y)
        Y += 12
        ...
        Y += 18
        e.Graphics.DrawString("Movimiento realizado por " & usu_copia, New Drawing.Font(tipografia, 7, FontStyle.Regular), Brushes.Black, 142.5, Y, sc)

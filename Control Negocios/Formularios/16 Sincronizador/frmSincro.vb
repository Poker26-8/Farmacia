...
#End Region
    ...
    Private Sub btn_configura_Click(sender As System.Object, e As System.EventArgs) Handles btn_configura.Click
            frmConfigSincro.Show()
            Timer_datos.Stop()
            Timer_reconecta.Stop()
      End Sub

      Private Sub frmSincro_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
    ...
    End Sub
      ...
      Private Sub Timer_reconecta_Tick(sender As System.Object, e As System.EventArgs) Handles Timer_reconecta.Tick
            Timer_reconecta.Stop()
            My.Application.DoEvents()
            If conecta() Then
                  Timer_reconecta.Stop()
      ...
      Private Sub Timer_datos_Tick(sender As System.Object, e As System.EventArgs) Handles Timer_datos.Tick
        Timer_datos.Stop()
      ...
      Private Sub get_sucursales()
        ...
        '        If .getDr(cnn, dr_Sucursales, "select COUNT(id) as numero_suc from sucursales", sinfo) Then
        '            num_Sucursales = dr_Sucursales("numero_suc").ToString
        '        Else
        '            num_Sucursales = 0
        '        End If

        '        If num_Sucursales > 1 Then
        '            If .getDr(cnn, dr_Sucursales, "select id from sucursales where Matriz = 1", sinfo) Then
        ...
        '        If .getDt(cnn, dt_Sucursales, "select * from sucursales", sinfo) Then
        '        End If
        '        cnn.Close()
        '    End If
        'End With
...
malo:
                ...
                If odata2.getDt(cnn2, dt, sSQL, sinfo) Then
                    For Each dr In dt.Rows
                        ...
                        Else
                            ...
                            ssqlinsertal = "Insert Into Productos(Codigo,Nombre,ProvPri,ProvRes,UCompra,UVenta,VentaMin,Departamento,Grupo,PrecioCompra,PorcentageMin,Porcentage,PrecioVenta,PrecioVentaIVA,PecioVentaMinIVA,IVA,Existencia,id_tbMoneda,PercentIVAret,NombreLargo,isr,ClaveSat,ClaveUnidadSat,MSeries,MCD,Multiplo,CodBarra,IIEPS,almacen3,nvoccorto,Porcentaje_Promo,PorMay,PorMM,PorEsp,PreMay,PreMM,PreEsp,CantMin,CantMay,CantMM,CantEsp,CantLta,CantMin2,CantMay2,CantMM2,CantEsp2,CantLta2,[Min],[Max]) " &
                                                      ...
                                                      "," & dr("PrecioCompra").ToString & ",'0',0," & dr("PorMay").ToString & "," & dr("PorMM").ToString & "," & dr("PorEsp").ToString & "," & dr("PreMay").ToString & "," & dr("PreMM").ToString & "," & dr("PreEsp").ToString &
                                                      ...
                                                      "," & dr("CantMM2").ToString & "," & dr("CantEsp2").ToString & "," & dr("CantLta2").ToString & "," & dr("Min").ToString & "," & dr("Max").ToString & ")"
                        ...
                        For Each dr In dt.Rows
                            If oData.getDr(cnn, dr2, "select Codigo,Existencia,Multiplo from Productos where Codigo = '" & dr("Codigo").ToString & "'", "drDOS") Then
                                ...
                                If oData.runSp(cnn, "update Productos set CargadoInv = 0, Existencia = Existencia + " & CDec(CDec(dr("Cantidad").ToString) * CDec(dr2("Multiplo").ToString)) & " where Codigo = '" & dr("Codigo").ToString & "'", sinfo) Then
                                    ...
                                    ssql3 = "insert into Cardex(Codigo,Nombre,Movimiento,Cantidad,Precio,Fecha,Usuario,Inicial,Final,Folio) values('" & dr("Codigo").ToString & "','" & dr("Descripcion").ToString & "','Ingreso por Compra Nube'," & CDec(CDec(dr("Cantidad").ToString) * CDec(dr2("Multiplo").ToString)) & ",'0','" & Now & "','Nube','" & MyExist & "','" & MyNewEsist & "','')"
            ...
            If odata2.dbOpen(cnn2, sTargetdSincro, sinfo) Then
                ...
                If odata2.getDt(cnn2, dt, sSQL, sinfo) Then
                    For Each dr In dt.Rows
                        ...
                        ssqlinsertal = "INSERT INTO Translados(Cargado,Nombre,Direccion,Usuario,FVenta,HVenta,FPago,FCancelado,Status,Comisionista,concepto,NUM_TRASLADO) " &
                                                   " VALUES (1,'TRASLADO','0','0','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','" & dr("Hora").ToString & "','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','PAGADO','" & dr("XD").ToString & "','SALIDA'," & dr("NumTraspasosS").ToString & ")"

                        If odata.runSp(cnn, ssqlinsertal, sinfo) Then

                            odata.getDr(cnn, dr2, "select max(Folio) as XD from Translados", "drdos")
            ...
            If odata4.dbOpen(cnn4, sTargetdSincro, sinfo) Then
                ...
                If odata4.getDt(cnn4, dt4, sSQL, sinfo) Then
                    For Each dr4 In dt4.Rows
                        ...
                        ssqlinsertal = "INSERT INTO TransladosDetalle(Folio, Codigo, Nombre, UVenta, Cantidad, CostVP, CostVUE, Precio, Total, PrecioSinIVA, TotalSinIVA, Fecha, Comisionista, Depto, Grupo, concepto, num_traslado)" &
                                        " VALUES (" & maxId & ",'" & dr4("Codigo").ToString & "','" & dr4("Nombre").ToString & "','" & dr4("UVenta").ToString & "'," & dr4("Cantidad").ToString & "," & dr4("Precio").ToString &
                                        "," & dr4("Precio").ToString & "," & dr4("Precio").ToString & "," & dr4("Total").ToString & "," & dr4("Precio").ToString & "," & dr4("Total").ToString & ",'" & Format(CDate(dr4("Fecha").ToString), "dd/MM/yyyy") & "','" & vardestino &
                        ...
                        For Each dr In dt.Rows
                            If oData.getDr(cnn, dr2, "select Codigo,Existencia,Multiplo from Productos where Codigo = '" & dr("Codigo").ToString & "'", "drDOS") Then
                                ...
                                If oData.runSp(cnn, "update Productos set Existencia = Existencia - " & CDec(CDec(dr("Cantidad").ToString) * CDec(dr2("Multiplo").ToString)) & ", CargadoInv = 0 where Codigo = '" & dr("Codigo").ToString & "'", sinfo) Then
                                    ...
                                    ssql3 = "insert into Cardex(Codigo,Nombre,Movimiento,Cant_Prod,Precio_prod,fecha,Usuario,Existencia,Diferencia,Folio) values('" & dr("Codigo").ToString & "','" & dr("Descripcion").ToString & "','Salida por Traspaso Nube'," & CDec(CDec(dr("Cantidad").ToString) * CDec(dr2("Multiplo").ToString)) & ",'0','" & Now & "','Nube','" & MyExist & "','" & MyNewEsist & "','')"
            ...
            If odata2.dbOpen(cnn2, sTargetdSincro, sinfo) Then
                ...
                If odata2.getDt(cnn2, dt, sSQL, sinfo) Then
                    For Each dr In dt.Rows
                        ...
                        ssqlinsertal = "INSERT INTO Translados(Cargado,Nombre,Direccion,Usuario,FVenta,HVenta,FPago,FCancelado,Status,Comisionista,concepto,NUM_TRASLADO) " &
                                                   " VALUES (1,'INGRESO','0','0','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','" & dr("Hora").ToString & "','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','" & Format(CDate(dr("Fecha").ToString), "dd/MM/yyyy") & "','PAGADO','" & dr("XD").ToString & "','ENTRADA'," & dr("NumTraspasosE").ToString & ")"
                        ...
                        For Each dr In dt.Rows
                            ...
                            If oData.getDr(cnn, dr2, "select Codigo,Existencia,Multiplo from Productos where Codigo = '" & Mid(dr("Codigo").ToString, 1, 6) & "'", "drDOS") Then
                                ...
                                If oData.runSp(cnn, sqlnew, sinfo) Then
                                    ...
                                    If Len(dr("Codigo").ToString) > 6 Then
                                        ssql3 = "insert into Cardex(Codigo,Nombre,Movimiento,Cant_Prod,Precio_prod,fecha,Usuario,Existencia,Diferencia,Folio) values('" & dr("Codigo").ToString & "','" & dr("Descripcion").ToString & "','Entrada por Traspaso Nube'," & CDec(dr("Cantidad").ToString) & ",'0','" & Now & "','Nube','" & MyExist & "','" & MyNewEsist & "','')"
                                    Else
                            ...
                            Else
                                ...
                                If odata2.getDr(cnn2, dr3, "Select * from productos where Codigo='" & dr("Codigo").ToString & "'", sinfo) Then
                                    ...
                                    If oData.runSp(cnn, ssqlinsertal, sinfo) Then
                                        ...
                                        MyExist = 0
                                        MyNewEsist = CDec(MyExist) + CDec(dr("Cantidad").ToString)
                                        ssql3 = "insert into Cardex(Codigo,Nombre,Movimiento,Cant_Prod,Precio_prod,fecha,Usuario,Existencia,Diferencia,Folio) values('" & dr3("Codigo").ToString & "','" & dr3("Nombre").ToString & "','Entrada por Traspaso Nube'," & dr("Cantidad").ToString & ",'0','" & Now & "','Nube','" & MyExist & "','" & MyNewEsist & "','')"
                                        oData.runSp(cnn, ssql3, sinfo)
    ...
    Private Sub subeTraspasoSalida()
        ...
        Dim cnn As MySqlClient.MySqlConnection = New MySqlClient.MySqlConnection
        Dim cnn2 As MySqlClient.MySqlConnection = New MySqlClient.MySqlConnection
        Dim sSQL As String = "Select * from Translados where Cargado=0"
        Dim ssqlinsertal As String = ""
        Dim ssql3 As String = ""
        Dim dt As New DataTable
        Dim dt2 As New DataTable
                ...
                If odata.getDt(cnn, dt, sSQL, sinfo) Then
                    For Each dr In dt.Rows
                        ...
                        If odata2.runSp(cnn2, ssqlinsertal, sinfo) Then
                            ...
                            ssql3 = "update Translados set Cargado=1 where Folio=" & dr("Folio").ToString
                            If odata.runSp(cnn, ssql3, sinfo) Then
    ...
    Private Sub subeTrasDetalle(ByVal Folio As String, ByVal maxId As String, ByVal numTras As String, ByVal vardestino As String)
        ...
        Dim cnn3 As MySqlClient.MySqlConnection = New MySqlClient.MySqlConnection
        Dim cnn4 As MySqlClient.MySqlConnection = New MySqlClient.MySqlConnection
        Dim sSQL As String = "Select * from TransladosDetalle where Folio=" & Folio
        Dim sSQL2 As String = ""
        Dim ssqlinsertal As String = ""
        Dim dt3 As New DataTable
        Dim dt4 As New DataTable
                        ...
                        Else
                            ...
                            If odata2.getDr(cnn2, dr2, "select * from proveedores where IdLocal = " & dr("Id").ToString & " and NumSuc = " & susursalr & " or NComercial = '" & dr("NComercial").ToString & "' and NumSuc = " & susursalr & " ", sinfo) Then
                                ...
                                ssqlinsertal = "update proveedores set `NComercial` = '" & dr("NComercial").ToString & "', `Compania` = '" & dr("Compañia").ToString & "', `RFC` = '" & dr("RFC").ToString & "', `CURP` = '" & dr("CURP").ToString & "', `Vendedor` = '" & dr("Vendedor").ToString & "', `Calle` = '" & dr("Calle").ToString &
                                                                "', `Colonia` = '" & dr("Colonia").ToString & "', `Delegacion` = '" & dr("Delegacion").ToString & "', `EntFed` = '" & dr("EntFed").ToString & "', `CP` = '" & dr("CP").ToString & "', `Tel1` = '" & dr("Tel1").ToString & "', `Tel2` = '" & dr("Tel2").ToString & "', `Fax` = '" & dr("Fax").ToString & "', `Ext1` = '" & dr("Ext1").ToString &
                                                                "', `Ext2` = '" & dr("Ext2").ToString & "', `Ext3` = '" & dr("Ext3").ToString & "', `Localizador` = '" & dr("Localizador").ToString & "', `PIN` = '" & dr("PIN").ToString & "', `TelMobil` = '" & dr("TelMobil").ToString & "', `PagWeb` = '" & dr("PagWeb").ToString & "', `Email` = '" & dr("Email").ToString & "', `NumClient` = '" &
                                                                dr("NumClient").ToString & "', `TelPart` = '" & dr("TelPart").ToString & "', `VendedorMail` = '" & dr("VendedorMail").ToString & "', `Saldo` = '" & dr("Saldo").ToString & "', `Credito` = '" & dr("Credito").ToString & "', `DiasCredito` = '" & dr("DiasCredito").ToString & "', `NumSuc` = " & susursalr &
                                                                ", Cargado = " & varcardado & " where Id = " & dr2("Id").ToString & " and NumSuc = " & susursalr & ""
                                If odata2.runSp(cnn2, ssqlinsertal, sinfo) Then
                                    banderaentra += 1
                                End If
                            Else
                                ...
                                ssqlinsertal = "INSERT INTO proveedores(`IdLocal`, `NComercial`, `Compania`, `RFC`, `CURP`, `Vendedor`, `Calle`, `Colonia`, `Delegacion`, `EntFed`, `CP`, `Tel1`, `Tel2`, `Fax`, `Ext1`, `Ext2`, `Ext3`, `Localizador`, `PIN`, `TelMobil`, `PagWeb`, `Email`, `NumClient`, `TelPart`, `VendedorMail`, `Saldo`, `Credito`, `DiasCredito`, `NumSuc`, `Cargado`) " &
                                                                ...
                                                                "','" & dr("Delegacion").ToString & "','" & dr("EntFed").ToString & "','" & dr("CP").ToString & "','" & dr("Tel1").ToString & "','" & dr("Tel2").ToString & "','" & dr("Fax").ToString & "','" & dr("Ext1").ToString & "','" & dr("Ext2").ToString &
                                                                "','" & dr("Ext3").ToString & "','" & dr("Localizador").ToString & "','" & dr("PIN").ToString & "','" & dr("TelMobil").ToString & "','" & dr("PagWeb").ToString & "','" & dr("Email").ToString & "','" & dr("NumClient").ToString & "','" & dr("TelPart").ToString &
                                                                "','" & dr("VendedorMail").ToString & "','" & dr("Saldo").ToString & "','" & dr("Credito").ToString & "','" & dr("DiasCredito").ToString & "'," & susursalr & ", " & varcardado & ")"
                                If odata2.runSp(cnn2, ssqlinsertal, sinfo) Then
                                    banderaentra += 1
                                End If
                            End If
                ...
                If odata2.getDt(cnn2, dt, sSQL, sinfo) Then
                    For Each dr In dt.Rows
                        ...
                        If odata.getDr(cnn, dr2, "select * from Proveedores where Id = " & dr("IdLocal").ToString & "", sinfo) Then
                            ...
                            ssqlinsertal = ""
                            ssqlinsertal = "update Proveedores set NComercial = '" & dr("NComercial").ToString & "', Compañia = '" & dr("Compania").ToString & "', RFC = '" & dr("RFC").ToString & "', CURP = '" & dr("CURP").ToString & "', Vendedor = '" & dr("Vendedor").ToString & "', Calle = '" & dr("Calle").ToString &
                                                          "', Colonia = '" & dr("Colonia").ToString & "', Delegacion = '" & dr("Delegacion").ToString & "', EntFed = '" & dr("EntFed").ToString & "', CP = '" & dr("CP").ToString & "', Tel1 = '" & dr("Tel1").ToString & "', Tel2 = '" & dr("Tel2").ToString & "', Fax = '" & dr("Fax").ToString & "', Ext1 = '" & dr("Ext1").ToString &
                                                          "', Ext2 = '" & dr("Ext2").ToString & "', Ext3 = '" & dr("Ext3").ToString & "', Localizador = '" & dr("Localizador").ToString & "', PIN = '" & dr("PIN").ToString & "', TelMobil = '" & dr("TelMobil").ToString & "', PagWeb = '" & dr("PagWeb").ToString & "', Email = '" & dr("Email").ToString & "', NumClient = '" &
                                                          dr("NumClient").ToString & "', TelPart = '" & dr("TelPart").ToString & "', VendedorMail = '" & dr("VendedorMail").ToString & "', Saldo = " & dr("Saldo").ToString & ", Credito = '" & dr("Credito").ToString & "', DiasCredito = " & dr("DiasCredito").ToString & ", Cargado = 1 where Id = " & dr2("Id").ToString & ""
                        ...
                        Else
                            ...
                            ssqlinsertal = ""
                            ssqlinsertal = "INSERT INTO Proveedores(NComercial, Compañia, RFC, CURP, Vendedor, Calle, Colonia, Delegacion, EntFed, CP, Tel1, Tel2, Fax, Ext1, Ext2, Ext3, Localizador, PIN, TelMobil, PagWeb, Email, NumClient, TelPart, VendedorMail, Saldo, Credito, DiasCredito, Cargado) " &
                                                              " VALUES ('" & dr("NComercial").ToString & "','" & dr("Compania").ToString & "','" & dr("RFC").ToString & "','" & dr("CURP").ToString & "','" & dr("Vendedor").ToString & "','" & dr("Calle").ToString & "','" & dr("Colonia").ToString &
                                                              "','" & dr("Delegacion").ToString & "','" & dr("EntFed").ToString & "','" & dr("CP").ToString & "','" & dr("Tel1").ToString & "','" & dr("Tel2").ToString & "','" & dr("Fax").ToString & "','" & dr("Ext1").ToString & "','" & dr("Ext2").ToString &
                                                              "','" & dr("Ext3").ToString & "','" & dr("Localizador").ToString & "','" & dr("PIN").ToString & "','" & dr("TelMobil").ToString & "','" & dr("PagWeb").ToString & "','" & dr("Email").ToString & "','" & dr("NumClient").ToString & "','" & dr("TelPart").ToString &
                                                              "','" & dr("VendedorMail").ToString & "','" & dr("Saldo").ToString & "','" & dr("Credito").ToString & "','" & dr("DiasCredito").ToString & "', 1)"
                        ...
                        Else
                            ...
                            Else
                                ...
                                ssqlinsertal = "INSERT INTO clientes(`IdLocal`, `Nombre`, `RazonSocial`, `Contacto`, `Tipo`, `RFC`, `CURP`, `Calle`, `Colonia`, `Delegacion`, `Entidad`, `CP`, `Telefono1`, `Telefono2`, `Fax`, `Ext1`, `Ext2`, `Ext3`, `Radio`, `Nip`, `Cel`, `Web`, `Email`, `TelParticular`, `ContactoMail`, `Cumple`, `Credito`, `Nota`, `Comisionista`, `DiasCredito`, `SuspVent`, `CNumberExt`, `CNumberInt`, `CPais`, `NumSuc`, `Cargado`) " &
                                                                ...
                                                                "','" & dr("Delegacion").ToString & "','" & dr("Entidad").ToString & "','" & dr("CP").ToString & "','" & dr("Telefono1").ToString & "','" & dr("Telefono2").ToString & "','" & dr("Fax").ToString & "','" & dr("Ext1").ToString & "','" & dr("Ext2").ToString &
                                                                "','" & dr("Ext3").ToString & "','" & dr("Radio").ToString & "','" & dr("Nip").ToString & "','" & dr("Cel").ToString & "','" & dr("Web").ToString & "','" & dr("Email").ToString & "','" & dr("TelParticular").ToString & "','" & dr("ContactoMail").ToString &
                                                                "','" & dr("Cumple").ToString & "','" & Replace(dr("Credito").ToString, ",", "") & "','" & dr("Nota").ToString & "','" & dr("Comisionista").ToString & "','" & dr("DiasCredito").ToString & "'," & dr("SuspVent").ToString & ",'" & dr("CNumberExt").ToString & "','" & dr("CNumberInt").ToString & "','" & dr("CPais").ToString & "'," & susursalr & ", " & varcardado & ")"
                                If odata2.runSp(cnn2, ssqlinsertal, sinfo) Then
                                    banderaentra += 1
                                End If
                            End If
                    ...
                    For Each dr In dt.Rows
                        ...
                        If odata.getDr(cnn, dr2, "select * from Clientes where Id = " & dr("IdLocal").ToString & "", sinfo) Then
                            ...
                            ssqlinsertal = "update Clientes set `Nombre` = '" & dr("Nombre").ToString & "', `RazonSocial` = '" & dr("RazonSocial").ToString & "', `Contacto` = '" & dr("Contacto").ToString & "', `Tipo` = '" & dr("Tipo").ToString & "', `RFC` = '" & dr("RFC").ToString & "', `CURP` = '" & dr("CURP").ToString & "', `Calle` = '" & dr("Calle").ToString &
                                                          "', `Colonia` = '" & dr("Colonia").ToString & "', `Delegacion` = '" & dr("Delegacion").ToString & "', `Entidad` = '" & dr("Entidad").ToString & "', `CP` = '" & dr("CP").ToString & "', `Telefono1` = '" & dr("Telefono1").ToString & "', `Telefono2` = '" & dr("Telefono2").ToString & "', `Fax` = '" & dr("Fax").ToString & "', `Ext1` = '" & dr("Ext1").ToString &
                                                          "', `Ext2` = '" & dr("Ext2").ToString & "', `Ext3` = '" & dr("Ext3").ToString & "', `Radio` = '" & dr("Radio").ToString & "', `Nip` = '" & dr("Nip").ToString & "', `Cel` = '" & dr("Cel").ToString & "', `Web` = '" & dr("Web").ToString & "', `Email` = '" & dr("Email").ToString & "', `TelParticular` = '" &
                                                          dr("TelParticular").ToString & "', `ContactoMail` = '" & dr("ContactoMail").ToString & "', `Cumple` = '" & dr("Cumple").ToString & "', `Credito` = '" & dr("Credito").ToString & "', `Nota` = '" & dr("Nota").ToString & "', `Comisionista` = '" & dr("Comisionista").ToString & "', `DiasCredito` = '" & dr("DiasCredito").ToString & "', `SuspVent` = '" & dr("SuspVent").ToString & "', `CNumberExt` = '" & dr("CNumberExt").ToString & "', `CNumberInt` = '" & dr("CNumberInt").ToString & "', `CPais` = '" & dr("CPais").ToString & "', Cargado = 1 where Id = " & dr2("Id").ToString & ""
                        ...
                        Else
                            ssqlinsertal = ""
                            ssqlinsertal = "INSERT INTO Clientes(`Id`, `Nombre`, `RazonSocial`, `Contacto`, `Tipo`, `RFC`, `CURP`, `Calle`, `Colonia`, `Delegacion`, `Entidad`, `CP`, `Telefono1`, `Telefono2`, `Fax`, `Ext1`, `Ext2`, `Ext3`, `Radio`, `Nip`, `Cel`, `Web`, `Email`, `TelParticular`, `ContactoMail`, `Cumple`, `Credito`, `Nota`, `Comisionista`, `DiasCredito`, `SuspVent`, `CNumberExt`, `CNumberInt`, `CPais`, `Cargado`) " &
                                                          " VALUES (" & dameMaxIdCli() + 1 & ",'" & dr("Nombre").ToString & "','" & dr("RazonSocial").ToString & "','" & dr("Contacto").ToString & "','" & dr("Tipo").ToString & "','" & dr("RFC").ToString & "','" & dr("CURP").ToString & "','" & dr("Calle").ToString & "','" & dr("Colonia").ToString &
                                                          "','" & dr("Delegacion").ToString & "','" & dr("Entidad").ToString & "','" & dr("CP").ToString & "','" & dr("Telefono1").ToString & "','" & dr("Telefono2").ToString & "','" & dr("Fax").ToString & "','" & dr("Ext1").ToString & "','" & dr("Ext2").ToString &
                                                          "','" & dr("Ext3").ToString & "','" & dr("Radio").ToString & "','" & dr("Nip").ToString & "','" & dr("Cel").ToString & "','" & dr("Web").ToString & "','" & dr("Email").ToString & "','" & dr("TelParticular").ToString & "','" & dr("ContactoMail").ToString &
                                                          "','" & dr("Cumple").ToString & "','" & Replace(dr("Credito").ToString, ",", "") & "','" & dr("Nota").ToString & "','" & dr("Comisionista").ToString & "','" & dr("DiasCredito").ToString & "'," & dr("SuspVent").ToString & ",'" & dr("CNumberExt").ToString & "','" & dr("CNumberInt").ToString & "','" & dr("CPais").ToString & "', 1)"
